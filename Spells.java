...
// Variable declarations --------------------------------------------------------------------------------------
		...
		ArrayList<String> dict = new ArrayList<String>(), level = new ArrayList<String>(), 
			level_p = new ArrayList<String>(), level_key = new ArrayList<String>(), 
			level_key_p = new ArrayList<String>(), w_spell = new ArrayList<String>(),
		...
		level = new ArrayList<String>(Arrays.asList(infile_level.nextLine().split("[^\\w']+"))); // level words list w/o punc.
		infile_level.nextLine();
		level_key = new ArrayList<String>(Arrays.asList(infile_level.nextLine().split("[^\\w']+"))); // level's answer key list w/o punc.
		infile_level = new Scanner(new File(args[1]));
		level_p = new ArrayList<String>(Arrays.asList(infile_level.nextLine().split("\\s+"))); // level words list with punc., no spaces
		infile_level.nextLine();
		level_key_p = new ArrayList<String>(Arrays.asList(infile_level.nextLine().split("\\s+"))); // Answer key with punc., no spaces
...
// Spells game algorithm -------------------------------------------------------------------------------------
		...
		for (int i=0; i<w_spell.size(); i++) {
			...
			level_p.set(opt -1, level_key_p.get(opt -1));
			r_spell.clear();
		}
		...
		for (int i=0; i<level_key.size(); i++) {
			System.out.print(level_key_p.get(i) + " ");
		}
		System.out.println();
